...
let get_cursor store id =>
  store >>= (fun store' => Store_ts.get_cursor store' path::["ts", id]);
...
let ts_since t l => List.filter (fun (ts, _) => ts >= t) l;

let ts_until t l => List.filter (fun (ts, _) => ts < t) l;

let ts_range t1 t2 l => ts_since t1 l |> ts_until t2;
...
let read_ts_data_all store id =>
  store >>=
    ...
    fun store' => Store_ts.read_all store' path::["ts", id];
